...
# script arguments ----
#
assertList(x = profile, len = 10)                  # ensure that profile is set


# load metadata ----
#
tifs <- make_rasternames(profile = profile, module = "initial landuse", step = "all")
...
mp_cover <- rast(x = str_replace(tifs$landcover, "_Y_", paste0("_", profile$years[1], "_")))
...
mp_rest <- rast(x = tifs$areaRestricted)
# test that resolution and extent match the profile
...
mp_suit <- rast(x = str_replace(tifs$suit_X_Y_Z, "_X_Y_Z_", paste0("_", target_ids, "_", profile$years[1], "_", target_tiles, "_")))
# test that resolution and extent match the profile
