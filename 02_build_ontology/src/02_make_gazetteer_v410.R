...
if(!testFileExists(gadmDir_v410)){
  if(!testFileExists(paste0(dataDir, "/input/gadm_410-levels.zip"))){
    stop("please store 'gadm_410-levels.zip' in '", dataDir, "/input/'")
  } else {
    if(!testFileExists(paste0(dataDir, "/input/gadm_410-levels.gpkg"))){
      message(" --> unpacking GADM basis")
      unzip(paste0(dataDir, "/input/gadm_410-levels.zip"))
    }
  }
}

gadm_layers <- st_layers(dsn = gadmDir_v410)
...
for(i in 1:4){
  ...
  if(i == 1){
    ...
    newLabel <- paste0("NAME_", i-1)
  }

  gazetteer <- new_mapping(new = newLabel,
